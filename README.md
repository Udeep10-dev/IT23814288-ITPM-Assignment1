...
|----------|-------|--------|--------|--------------|
| Positive Functional | 24 | 23 | 1 | 95.8% |
| Negative Functional | 10 | 10 | 0 | 100% |
| UI Tests | 1 | 1 | 0 | 100% |

### Known Issues

1. **Pos_Fun_0008: Question form conversion**
   - **Status:** FAILED
   - **Input:** `mee parigaNakaya nisi aakaarava vaeda karanavadha ...`
   - **Expected:** `මේ පරිගණකය නිසි ආකාරව වැඩ කරනවද ?`
   - **Actual:** Empty output
   - **Issue:** The translator fails to process this specific input pattern
   - **Screenshot:** Available at `test-results/screenshots/Pos_Fun_0008.png`
...
1. **Test Data Source**: Excel file (`test-data/IT23814288.xlsx`) contains:
   - Test case IDs
   - Test case names
   - Input text (Singlish)
...
2. **Test Execution Flow**:
   ...
   Read Excel → For Each Test Case:
                   ...
                   └── Log results & take screenshots (if failed)
   ```

3. **Test Types**:
   - **Positive Tests**: Verify correct behavior (should PASS)
   - **Negative Tests**: Document edge case behavior (document actual output)
   - **UI Tests**: Validate interface functionality

### Key Selectors

The automation interacts with Swift Translator using these selectors:

```javascript
// Input field (where you type Singlish)
page.getByRole('textbox', { name: 'Input Your Singlish Text Here.' })

// Output field (where Sinhala translation appears)
page.locator('.w-full.h-80.p-3.rounded-lg.ring-1.ring-slate-300.whitespace-pre-wrap')
```
...
- **Headless Mode**: `false` (browser visible by default)
- **Screenshots**: Captured only on failure
- **Video**: Recorded only for failed tests
...
### Example 1: Simple Sentence (Positive Test)

**Test ID:** Pos_Fun_0001  
**Category:** Daily language usage, Simple sentence, Short input  
**Input:** `mama pothak kiyavanavaa`  
**Expected:** `මම පොතක් කියවනවා`  
**Result:** ✅ PASS

### Example 2: Mixed Language (Positive Test)

**Test ID:** Pos_Fun_0022  
**Category:** Mixed Singlish + English, Long input  
**Input:** `mee mama adha raeeta project proposal eka gaena poddak kathaa karaganna oona nisaa podi meeting ekak arrange karaa. 7.00 pm valata karanna thamayi dhaenata plan eka thiyennee. MS teams valin thamayi karannee. kattiya hamoomə anivaaryen enna onee. mama meeting link eka thava tikakin group ekata send karannam.`  
**Expected:** Full Sinhala translation with English terms preserved  
**Result:** ✅ PASS

### Example 3: Edge Case (Negative Test)

**Test ID:** Neg_Fun_0009  
**Category:** URL/Email Corruption  
**Input:** `https://courseweb.sliit.lk '`  
**Expected:** `https://courseweb.sliit.lk '`  
**Actual:** `හ්ට්ට්ප්ස්://cඔඋර්සෙwඑබ්.ස්ලීට්.ල්ක් '`  
**Result:** ✅ DOCUMENTED (system corrupts URLs as expected for negative test)
